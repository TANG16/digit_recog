...
for i = 1:length(classid),
    ...
    if(config.KERNEL_TYPE == 0),
        [l,a,d] = predict(class_label,sparse(te_feats),models{i}.svm);
        [l2,a2,d2] = predict(class_label,sparse(d),models{i}.logist,'-b 1');
        probs(:,i) = d2(:,find(models{i}.logist.Label==2));        
    else
        [l,a,d] = svmpredict(class_label,sparse(te_feats),models{i}.svm,'-b 1');
