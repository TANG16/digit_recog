...
%         svmstr = sprintf('-t %i -d %i -r %.2f -g %.2f -b 1',...
%                          config.KERNEL_TYPE, config.LIBSVM_d, config.LIBSVM_r,...
%                          config.LIBSVM_g);
        svmstr = sprintf('-t %i -d %i -r %.2f -g %.2f -c %i -w2 %.2f -b 1',...
                         config.KERNEL_TYPE, config.LIBSVM_d, config.LIBSVM_r,...
    ...
    if(config.KERNEL_TYPE==0),
        model{i}.svm = train(class_labels,sparse(tr_feats),svmstr);
        [l,a,d] = predict(class_labels,sparse(tr_feats),model{i}.svm);
        model{i}.logist = train(class_labels,sparse(d),svmstr_logist);
    else
